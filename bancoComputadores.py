...
def salvar_alteracoes(usuario, tipo_equipamento, ip, licencas, id):
    conexao = sqlite3.connect('banco_de_dados_equipamentos_licencas.db')
    cursor = conexao.cursor()
    cursor.execute('UPDATE equipamento SET usuario = ?, tipo_equipamento = ?, ip = ?, licenca_id = ? WHERE id = ?', (usuario,tipo_equipamento,ip,licencas,id))
